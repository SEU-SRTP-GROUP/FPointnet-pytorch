...
class kitti_object(object):
    ...
    def __init__(self, ROOT_DIR, split='val'):
        ...
        self.root_dir = os.path.join(ROOT_DIR, 'data\\kitti\\object')
        self.split = split
        ...
        self.label_dir = os.path.join(ROOT_DIR, 'models_stereo\\data')
...
def dataset_viz():
    ...
    val_idx = [line.rstrip() for line in open(os.path.join(ROOT_DIR, 'data\\kitti\\splits\\val.txt'))]

    for i in range(len(val_idx)):
        # Load data from dataset
        data_idx = int(str(val_idx[i]))
        #data_idx = int(str(val_idx[22]))
