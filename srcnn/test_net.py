  ...
  with torch.no_grad():
    ...
    for i in range(num_images):
      ...
      for j in xrange(1, imdb.num_classes):
        #torch.nonzero 返回一个包含tensor中所有非零元素的tensor
          ...
          if boxes_all.dim() > 0:
            ...
            for solved_idx in range(succ.size(0)):
              if succ[solved_idx] > 0: # succ
                ...
                kitti_utils.write_detection_results(imdb._classes[j], result_dir, image_number, calib, box_left,xyz, dim, theta, score)
                kitti_utils.write_rgbdetection_results(j,result_dir, image_number, calib, box_left,\
                                                    xyz, dim, theta, score)
