...
class resnet(_StereoRCNN):
  ...
  def _init_modules(self):
    ...
    # Top layer
    self.RCNN_toplayer = nn.Conv2d(512, 128, kernel_size=1, stride=1, padding=0)  # reduce channel

    # Smooth layers
    self.RCNN_smooth1 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1)
    self.RCNN_smooth2 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1)
    self.RCNN_smooth3 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1)

    # Lateral layers
    # 这里拼上vgg后通道数不匹配的
    self.RCNN_latlayer1 = nn.Conv2d( 512, 128, kernel_size=1, stride=1, padding=0)
    self.RCNN_latlayer2 = nn.Conv2d( 256, 128, kernel_size=1, stride=1, padding=0)
    self.RCNN_latlayer3 = nn.Conv2d( 128, 128, kernel_size=1, stride=1, padding=0)
