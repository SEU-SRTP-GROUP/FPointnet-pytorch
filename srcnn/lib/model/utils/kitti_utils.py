...
#---------------------------------------------------- Data Writing -------------------------------------------------#
def write_detection_results(classname, result_dir, file_number, calib, box_left, pos, dim, orien, score):
...
def write_rgbdetection_results(type, result_dir, file_number, calib, box_left, pos, dim, orien, score):
    '''One by one write detection results to KITTI format label files.
    '''
    if result_dir is None: return
    result_dir = result_dir + '/data'
    ...
    output_str = 'dataset/KITTI/object/training/image_2/%s' % file_number
    output_str+= '.png '
    output_str += '%d ' % type
    alpha = orien - m.pi/2 + m.atan2(-pos[0], pos[2])
    output_str += '%f %d %d %d %d \n' % (score, box_left[0],box_left[1],box_left[2],box_left[3])

    # Write TXT files
    if not os.path.exists(result_dir):
        os.makedirs(result_dir)
    pred_filename = 'rgb_detection_val.txt'
    with open(pred_filename, 'a') as det_file:
        det_file.write(output_str)
