...
class SrcnnTrainer : 
    ...
    def train(self, restore_model_dir = None, train_mode = "default"):
    ...
    def _train(self):
        ...
        sampler_batch = sampler(train_size, self.config.batch_size)

        dataset = roibatchLoader(roidb, ratio_list, ratio_index, self.config.batch_size, \
                                 imdb.num_classes, training=True)

        dataloader = torch.utils.data.DataLoader(dataset, batch_size=self.config.batch_size,
                                                 sampler=sampler_batch, num_workers=self.config.num_workers)
        ...
        # 是否需要续着上次的训练接着 读之前的参数
        if self.config.resume:
            # 这个地方到时候要重写，比如应该是读与训练模型
            load_name = os.path.join(output_dir,
                                     'stereo_rcnn_{}_{}.pth'.format(self.config.checkepoch, self.config.checkpoint))
        ...
        # 分配显卡的使用device，不用管
        stereoRCNN.cuda()
        iters_per_epoch = int(train_size / self.config.batch_size)
        for epoch in range(self.config.start_epoch, self.config.max_epochs + 1):
            print('epoch: %d' % epoch)
            stereoRCNN.train()
            start = time.time()
            if epoch % (self.config.lr_decay_step + 1) == 0:
                adjust_learning_rate(optimizer, self.config.lr_decay_gamma)
                lr *= self.config.lr_decay_gamma
            ...
            for step in range(iters_per_epoch):
                ...
                '''
                log_string('\t\t\trpn_loss_cls: %.4f, rpn_loss_box_left_right: %.4f, RCNN_loss_cls: %.4f, RCNN_loss_bbox %.4f,RCNN_loss_dim_orien %.4f, RCNN_loss_kpts %.4f' \
                      %(rpn_loss_cls, rpn_loss_box_left_right, RCNN_loss_cls, RCNN_loss_bbox, RCNN_loss_dim_orien, RCNN_loss_kpts))
                '''
                uncert_data = uncert.data
                log_string('uncert: %.4f, %.4f, %.4f, %.4f, %.4f, %.4f' \
                           % (uncert_data[0], uncert_data[1], uncert_data[2], uncert_data[3], uncert_data[4],
                              uncert_data[5]))
